...
export default function HomePage() {
  ...
  async function handleEarnAndHarvest() {
    setStage("Harvesting");
    try {
      await earnAndHarvest();
      ...
      console.error("Failed to earn and harvest:", error);
    }
  }

  async function handleEmergencyWithdrawFromStrategy() {
    setStage("Withdrawing");
    try {
      await emergencyWithdrawFromStrategy();
  ...
  async function handlePerformClose() {
    setStage("Closing");
    try {
      await resetVaultFlags();
      await performClose();
      console.log("Closing");
      setTimeLeft(Math.floor(Date.now() / 1000) + 10);
      setStagePassed(5);
